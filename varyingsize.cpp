...
int main() {
    ...
    for (int L = 40; L <= 120; L += 20) { // size-loop
        IsingModel IsingModel(L, "mcc5" + to_string(L), cycles); // defines environment, data file will have tha basename "mcc5"
        IsingModel.initialize(); // spins initialized in raodnom directions, up or down
        for (double T = 2.0; T <= 2.5; T += 0.01) { // temperature loop for each lattice size
